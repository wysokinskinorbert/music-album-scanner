...
class OfflineRecognitionService {
  ...
  String? get modelPath {
    // Return a placeholder; real path comes from ModelDownloadManager
    return null;
  }
  ...
  Future<OfflineRecognitionResult> recognize(String imagePath) async {
    ...
    if (modelResult != null && modelResult.confidence >= 0.6) {
      return modelResult;
    }
  ...
  Future<OfflineRecognitionResult?> _tryModelRecognition(String imagePath) async {
    ...
    try {
      ...
      return OfflineRecognitionResult(
        recognized: result.confidence >= 0.6,
  ...
  /// Index a single album (alias for addToIndex).
  Future<void> indexAlbum(Album album) async {
    await addToIndex(album);
  }
